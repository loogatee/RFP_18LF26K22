...
void CMDS_Process(void)
{
    ...
    case CMDSM_WAITFORLINE:
        ...
        if( cmds_InpPtr[0] == 'm' && cmds_InpPtr[1] == 'd')
        {
            signal_done = cmds_MD( MD_INIT );
        }
    ...
    case CMDSM_MEMDUMP:
        ...
        signal_done = cmds_MD( MD_PROCESS );
        break;
        
    }

    if( signal_done == TRUE ) { SerialDbg_SignalCmdDone(); }
}
...
static bool cmds_MD( u8 state )
{
    ...
    case MD_PROCESS:
        ...
        if( cmds_completion == 1 )
        {
            ...
            SerialDbg_Sout( SERD_OTYPE_ROM, (char *)"\r\n", &cmds_completion, 0 );
